...
#SBATCH --output=results/final/run_gmm_omp_mpi_batch_output20M10Ft5K.txt
#SBATCH --error=results/final/run_gmm_omp_mpi_batch_error20M10Ft5K.txt
...
mpic++ -fopenmp -I$(spack location -i eigen)/include gmm_mpi_omp.cpp -o ./build/gmm_mpi_omp20M
if [ $? -ne 0 ]; then
...
for CONFIG in "${CONFIGURATIONS[@]}"; do
    ...
    for ((i=1; i<=NUM_RUNS; i++)); do
        ...
        RESULT=$(srun --nodes=$NUM_NODES --ntasks=$NUM_TASKS --cpus-per-task=$CPUS_PER_TASK ./build/gmm_mpi_omp20M)
