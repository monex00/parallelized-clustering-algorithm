...
void computeInverseMatrices(
    ...
    for (int i = 0; i < batchSize; i++) {
        double det = 1.0;  // Inizializza a 1.0 per il prodotto
        int swaps = 0;
...
/* 
        ...
        double* h_invCovMatrices = (double*)malloc(k * d * d * sizeof(double));
        CUDA_CHECK(cudaMemcpy(h_invCovMatrices, d_invCovMatrices, k * d * d * sizeof(double), cudaMemcpyDeviceToHost));
        for (int j = 0; j < d; ++j) {
            for (int l = 0; l < d; ++l) {
