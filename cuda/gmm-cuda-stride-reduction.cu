...
/*     double feature_means[d];
    ...
    for (int iter = 0; iter < maxIter; ++iter)
    {
        ...
        int maxSharedMemory = 49152; // Memoria condivisa massima (48 KB per molte GPU moderne)
        ...
        float iterTime = 0;
        CUDA_CHECK(cudaEventElapsedTime(&iterTime, startIter, stopIter));
        printf("Iteration %d: time= %f\n", iter, iterTime); // in milliseconds
